...
namespace osu.Game.Screens.Play.HUD
{
    public class CopyPastas : RollingCounter<double>, ISkinnableDrawable
    {
        ...
        private string[] copypastawords;

        private string[] splitchars = {"\n","\t"," ", "\n "};

        private int currentindex = 0;
        
        private string currentword = "According";
        ...
        // private void cleardearray()
        // {
        //     copypastawords = copypastawords.Where(word => String.IsNullOrWhiteSpace(word)).ToArray();
        // }
        ...
        private void load(OsuColour colours, ScoreProcessor scoreProcessor)
        {
            ...
            copypastawords = bee_movie.Split(splitchars, System.StringSplitOptions.RemoveEmptyEntries).Where(word => !String.IsNullOrWhiteSpace(word)).ToArray();
            if(copypastawords.Length > 0)
            {
                currentword = copypastawords[0];
        ...
        protected void OnNewJudgement(JudgementResult judgement){
            ...
            SetCountWithoutRolling(judgement.TimeAbsolute);
            if(copypastawords.Length > 0)
            {
                currentindex = (currentindex+1)%copypastawords.Length;
                currentword = copypastawords[currentindex];
            }
        ...
        {   
            // return $@"{currentword} [{currentindex}] ({currentword.Length}) ({String.IsNullOrWhiteSpace(currentword)})";
            return $@"{currentword}";
